...
async function pushChanges(cfg, local, shadow) {
  ...
  const table = (name, toRow, localList, shadowList, keyed) => {
    const localRows = localList.map(toRow);
    const shadowRows = (shadowList || []).map(toRow);
    const { changed, removed } = diffById(localRows, shadowRows);
    if (changed.length) { jobs.push(upsert(cfg, name, changed)); touched.push(name + ' +' + changed.length); }
    if (removed.length) { jobs.push(removeByIds(cfg, name, removed)); touched.push(name + ' -' + removed.length); }
  };

  table('houses',   (h, i) => toRowHouse(h, i),   local.houses,   shadow.houses);
  table('events',   (e, i) => toRowEvent(e, i),   local.events,   shadow.events);
  table('students', toRowStudent,                 local.students, shadow.students);
  table('results',  toRowResult,                  local.results,  shadow.results);

  // Sheets have a composite key, so they are diffed on the local map key.
  const sheetRows = [];
  ...
  if (sheetRows.length) {
    jobs.push(upsert(cfg, 'sheets', sheetRows, 'event_id,division'));
    touched.push('sheets +' + sheetRows.length);
  }
  const goneSheets = Object.keys(shadow.sheetMeta || {}).filter(k => !(local.sheetMeta || {})[k]);
  goneSheets.forEach(k => {
    const cut = k.indexOf('::');
    jobs.push(rest(cfg, 'sheets?event_id=eq.' + encodeURIComponent(k.slice(0, cut)) +
  ...
  if (settingRows.length) { jobs.push(upsert(cfg, 'settings', settingRows)); touched.push('settings +' + settingRows.length); }
